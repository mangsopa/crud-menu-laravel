(function ($) {
    ...
    jQuery(document).ready(function($){
        ...
        // mobile navbar
        var navbarOpem = false;
        $('.navbar-toggler').on('click', function(){
            if(navbarOpem == false) {
                navbarOpem = true;
                $('html').css('overflow', 'hidden');
                $('body').css('right', '240px');
                $('.header').find('.navbar').css('right', '0');
            } else {
                navbarOpem = false;
                $('html').css('overflow', 'scroll');
                $('body').css('right', '0');
                $('.header').find('.navbar').css('right', '-240px');
            }
        });
        ...
        var bannerSlidesDots = $('.banner-slider').find('.owl-dots');
        bannerSlidesDots.css('bottom', features_section_height / 2 + 30);
